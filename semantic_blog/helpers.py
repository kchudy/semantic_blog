...
def get_article_enhancements(article):
    g = Graph()
    g.parse(get_content_meta_rdf(article.content.encode('utf-8')))

    predicate_name = URIRef(
        'http://fise.iks-project.eu/ontology/selected-text')
    predicate_entity_label = URIRef(
    ...
    for subject in subjects:
        is_enhancement = get_first(g.triples((subject, predicate_type, URIRef(
        ...
        if is_enhancement:
            ...
            value = get_first(
                g.objects(subject=subject, predicate=predicate_name))

            if not value:
                value = get_first(g.objects(subject=subject,
                                            predicate=predicate_entity_label))

            if value:
                # there are enhancements with no value - ignore them
                enhancement.value = value

                entity_name = get_first(
                    g.objects(subject=subject, predicate=predicate_entity_ref))

                if entity_name:
                    enhancement.comment = get_first(
                        g.objects(subject=entity_name,
                                  predicate=predicate_comment))
                    entity_tags = map(save_tag, g.objects(subject=entity_name,
...
def get_tag_value(tag_raw):
    return urllib.unquote(
        str(tag_raw).replace('http://dbpedia.org/resource/Category:',
                             '').replace('_', ' '))


def save_tag(value):
    tag_value = urllib.unquote(
        str(value).replace('http://dbpedia.org/resource/Category:',
                           '').replace('_', ' '))
